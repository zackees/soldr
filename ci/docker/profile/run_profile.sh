...
log "==> installing rust toolchain 1.98.1 ..."
rustup toolchain install 1.98.1 --profile minimal --no-self-update 2>&1 \
    | tee -a "${OUT_DIR}/build.log" | tail -3
rustup default 1.98.1 2>&1 | tee -a "${OUT_DIR}/build.log" | tail -3

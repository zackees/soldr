...
class DownloadLargeAssetTests(unittest.TestCase):
    def run_helper(
        self, mock_body: str, payload: bytes, *, size: int | None = None
    ):
        with tempfile.TemporaryDirectory() as temp:
            ...
            mock.chmod(0o755)
            env = os.environ | {"SOLDR_DOWNLOAD_CURL_BIN": str(mock), "SOLDR_DOWNLOAD_RETRIES": "2"}
            command = [
                "bash", str(HELPER), "--url", "https://example.test/a", "--output",
                str(output), "--sha256", hashlib.sha256(payload).hexdigest(),
            ]
            if size is not None:
                command += ["--expected-size", str(size)]
            result = subprocess.run(command, capture_output=True, text=True, env=env)
            return result, output.exists(), output.read_bytes() if output.exists() else b""

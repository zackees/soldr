...
EXECUTION_MODES = frozenset({"native", "x86_64-rosetta", "x86_64-recovery"})
...
def command_argv(execution: str, command: Sequence[str]) -> list[str]:
    ...
    packaged command. x86_64-recovery instead ships work into a
    zackees/docker-mac-x64 Recovery guest as one whole boot-time script
    (soldr#3076) -- there is no per-command exec to rewrite an argv for, so
    callers in that mode never reach this function; see
    `ci/macos_recovery_run.py` and `_ci-target-run.yml` instead.
    """
    ...
    if execution == "x86_64-recovery":
        raise ValueError(
            "x86_64-recovery has no per-command argv rewrite; it ships one "
            "whole guest script via ci/macos_recovery_run.py instead"
        )
    return list(command)

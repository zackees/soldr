...
| File | Role |
|------|------|
| `Dockerfile` | Single-stage `rust:1.98.1-alpine` image. Copies `Cargo.toml`/`Cargo.lock`/`rust-toolchain.toml` + `crates/` from the build context (the repo root), builds `soldr-cli --release`, installs the binary to `/usr/local/bin/soldr`, and `cargo clean`s to shrink the image. |

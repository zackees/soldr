...
- **`ci-test` is also a frozen native verb** (soldr#2867): it is the prescribed host-validation DAG, not shorthand for `test` and not an external tool. It maximizes sharing by compile domain: stable host Clippy subsumes `soldr cargo check`; after Clippy, Nextest test-profile compilation completes before compiler-free Nextest execution overlaps the serial Dylint libraries -> workspace-analysis -> UI-test branch; doctests join both branches and retain their rustdoc family; and all six Dylints use separate target trees keyed by the one exact nightly declared by every lint manifest. Unset `CARGO_BUILD_JOBS` and `SOLDR_JOBS` remain unset, explicit values are preserved byte-for-byte, and only `NEXTEST_TEST_THREADS` defaults to one. The overlap has one compiler-bearing Cargo branch; harmless Cargo/rustc metadata probes remain allowed, but codegen or other compiler work from Nextest execution is a scheduler-invariant failure rejected before compiler admission or spawn. Cgroup memory observations remain telemetry and never impose a global Cargo job cap. Dylint is allowed its own toolchain, but it must never reuse or invalidate the stable target tree. `--explain-plan --format json` is the versioned machine-readable contract.

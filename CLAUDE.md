...
| `_bootstrap-e2e.yml` "Build soldr-cli", `setup-soldr-action.yml` "Build through soldr" | They build soldr through a *released* soldr (the setup-soldr pin / the action's default, 0.9.15 on 2026-09-14) that predates soldr#3211, so uncapped the compiler children can be SIGTERMed mid-compile (soldr#3210). Step-scoped since soldr#3148; lift once the pinned release carries soldr#3211. |

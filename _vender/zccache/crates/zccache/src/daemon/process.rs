...
/// Streaming variant of [`tokio_command_output_with_priority`] (Phase 5b2,
/// soldr#983).
///
/// Spawns the child the same way as the buffered helper but reads its
/// stdout / stderr pipes concurrently and forwards each non-empty buffer
/// chunk to `sink` as it arrives, in addition to accumulating the bytes
/// into the returned [`Output`]. The accumulation is required because
/// the on-disk artifact store, MSVC `/showIncludes` filtering, and the
/// rustc-error replay all consume the captured buffers downstream of
/// the spawn — see `pipeline::store_outcome`. The architectural win is
/// the IPC ship: the soldr daemon no longer waits for the child to
/// exit before chunking the captured output through its named-pipe /
/// Unix-socket framing layer; the chunks land on the wrapper's
/// stdout/stderr in lockstep with the child's writes.
///
/// The `compile_concurrency` semaphore semantics are unchanged — the
/// permit is acquired by the caller (`pipeline::compile_exec`) before
/// invoking this helper and held until the future resolves, exactly
/// as the buffered variant required.
pub(crate) async fn tokio_command_streaming_with_priority(
    cmd: &mut tokio::process::Command,
    priority: CompilePriority,
    sink: crate::daemon::server::StreamingSink,
) -> io::Result<Output> {
    use std::process::Stdio;
    ...
    let mut stdout_pipe = child.stdout.take().expect("Stdio::piped above");
    let mut stderr_pipe = child.stderr.take().expect("Stdio::piped above");

    // Spawn concurrent pumps. 64 KiB read chunks match the soldr-side
    // CHUNK_BYTES so each pump iteration produces at most one wire
    // frame downstream. Each pump returns the accumulated buffer so
    // the caller can hand it to the on-disk artifact store.
    ...
    let stdout_task = tokio::spawn(async move {
        ...
        loop {
            ...
            stdout_sink.send_stdout(buf[..n].to_vec()).await;
        }
        Ok::<Vec<u8>, io::Error>(acc)
    });
    ...
    let stderr_task = tokio::spawn(async move {
        ...
        loop {
            ...
            acc.extend_from_slice(&buf[..n]);
            stderr_sink.send_stderr(buf[..n].to_vec()).await;
        }
        Ok::<Vec<u8>, io::Error>(acc)
    });
    ...
    let stdout = stdout_task
        .await
        .map_err(|e| io::Error::other(format!("stdout pump join failed: {e}")))??;
    let stderr = stderr_task
        .await
        .map_err(|e| io::Error::other(format!("stderr pump join failed: {e}")))??;

...
fn manual_redirect_limit_accepts_five_then_final_response() {
    runtime().block_on(async {
        let url = serve_redirect_chain(MAX_REDIRECT_HOPS).await;
        let client = reqwest::Client::builder()
            .redirect(reqwest::redirect::Policy::none())
            .build()
            .unwrap();
        let response = send_with_hop_timeout(
            &client,
            &url,
...
fn manual_redirect_limit_rejects_sixth_and_redacts_unsafe_target() {
    runtime().block_on(async {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .unwrap();
        let error = send_with_hop_timeout(
            &client,
            &url,
            |client, url| client.get(url),
            Duration::from_secs(2),
        ...
        let unsafe_error = resolve_redirect_url(

...
impl NestedCargoMonitor {
    pub(crate) fn new(root_pid: u32, outer_target: Option<&Path>, enabled: bool) -> Option<Self> {
        Self::new_if_enabled(
            root_pid,
            outer_target,
            enabled && !nested_cargo_permitted(std::env::var(PERMIT_ENV).ok().as_deref()),
        )
    }

    fn new_if_enabled(root_pid: u32, outer_target: Option<&Path>, enabled: bool) -> Option<Self> {
        if !enabled {
            return None;
        }
        Some(Self {

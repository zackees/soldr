...
pub(crate) async fn prepare(
    ...
    // Tier 1: an already-prepared parent process handed us its exact
    // plan via the retained environment. Highest precedence, unchanged.
    if let Some(channel) = non_empty_env(TOOLCHAIN_ENV_VAR) {
        let plan = plan_from_retained_environment(&channel)?;
        ensure_plan_verified(&plan)?;
        return Ok(plan);
    }
    // Tier 2: an explicit, fully-specified configured environment.
    // Unchanged.
    ...
    // Tier 3: resolve the requested Rust version, then try the
    // disk-backed prepared-plan marker BEFORE paying for the catalogue
    // HTTP fetch + rustup subprocess probes. This is the warm-run fast
    // path: a fresh, valid marker whose toolchain directory is still
    // present on disk lets a top-level `soldr cargo dylint` skip
    // network + subprocess entirely.
    let version = requested_rust_version(requested_channel, workspace_root)?;

    if !truthy_env(REVERIFY_ENV_VAR) {
        if let Some(plan) = load_prepared_marker(&version) {
            eprintln!(
    ...
    .await?;
    let plan = select_from_map(&bytes, &version)?;
    ensure_plan_verified(&plan)?;
...
/// Shared verification tail for every [`prepare`] tier: unless a nested
/// child process already inherited this exact identity via
/// `SOLDR_DYLINT_PREPARED_IDENTITY`, confirm the toolchain + required
/// components are installed and that the observed compiler identity
/// matches the plan.
fn ensure_plan_verified(plan: &DylintToolchainPlan) -> Result<(), SoldrError> {

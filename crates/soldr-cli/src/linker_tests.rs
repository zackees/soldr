...
#[test]
fn nothing_falls_back_to_fast_reld() {
...
fn empty_env_string_falls_back_to_default() {
    ...
    // An empty env string is not a value; `from_str("")` maps it to `Default`
    // (the explicit no-injection choice), which is distinct from the reld
    // default reached only when the env var is entirely unset.
...
fn proc_macro_on_msvc_loses_the_injected_reld() {
    // `fast` now injects reld (which bridges to lld-link on MSVC), so it must
    // be stripped for proc-macro dylibs exactly like rust-lld.

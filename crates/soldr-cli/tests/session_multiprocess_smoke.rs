...
//! the Docker harness covers); it runs under a generous nextest budget and
...
fn session_compile_over_broker_multiprocess() {
    ...
    let daemon =
        common::soldr_bin().with_file_name(format!("soldr-daemon{}", std::env::consts::EXE_SUFFIX));
...
#[test]
fn issue_2476_handed_off_compile_survives_broker_death_and_daemon_is_readopted() {
    if matches!(

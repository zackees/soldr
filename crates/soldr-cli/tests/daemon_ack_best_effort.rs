...
//!
//! `CONTROL_CONNECTOR` is a `OnceLock`, so it can be installed once per
//! process. That is why this is its own integration target rather than a case
//! inside `cli_daemon_target_touch.rs`, whose other tests drive a real daemon.

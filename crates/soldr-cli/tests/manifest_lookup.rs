...
fn manifest_hit_returns_pinned_url() {
    ...
    let hit = idx
        ...
        .expect("manifest hit");
    // Through the accessor, not the raw field: soldr#2843 made `url`
    // optional and added `urls`, and `direct_url()` is what the resolver
    // calls -- so this keeps asserting what production reads.
    assert_eq!(
        hit.direct_url(),
...
fn manifest_sha256_mismatch_is_hard_error() {
    ...
    let entry = ManifestEntry {
        ...
        // string: soldr#2843 made `url` optional, so a multipart entry carries
        // `parts` and no single URL, and "absent" stopped meaning "empty".
        url: None,
        urls: Vec::new(),
        parts: Vec::new(),
        size_bytes: None,

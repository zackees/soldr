...
events. The stable broker owns daemon process creation (soldr#2900 deleted the
last unreachable direct-spawn helpers that predated it).
...
| `tests/` | the former inline test modules, one file each |

## Spawn environment

`spawn_env.rs` supplies the narrow environment overlay the stable broker uses
when it launches a daemon generation. Composing that overlay is a distinct
concern from the broker's process-creation boundary, so it stays its own file.

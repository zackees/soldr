//! Linux atomic replacement and open-file retirement.
...
/// No upgrade needed on Linux: an open file can be unlinked directly.
pub fn open_for_retire(file: File) -> io::Result<File> {
    Ok(file)
}

/// Retire `file`: drop the handle and run the caller's remove. Linux

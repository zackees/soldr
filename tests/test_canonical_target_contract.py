...
def workflow_job(text: str, job: str) -> str:
    match = re.search(rf"^  {re.escape(job)}:\s*$(.*?)(?=^  [\w-]+:\s*$|\Z)", text, re.MULTILINE | re.DOTALL)
...
def test_runtime_and_manifest_mirrors_match_contract() -> None:
    ...
    manifest_section = (ROOT / "Cargo.toml").read_text(encoding="utf-8").split(
        "[workspace.metadata.soldr]", 1
    )[1].split("\n[", 1)[0]
    manifest_targets = re.findall(r'"([^-"\s]+(?:-[^"\s]+){2,})"', manifest_section)

    assert core == expected_triples, "CANONICAL_TARGETS drifted from canonical-targets.json"
    assert aliases == expected_aliases, "CANONICAL_ALIASES drifted from canonical-targets.json"
    assert manifest_targets == expected_triples, "Cargo target metadata drifted from canonical-targets.json"
...
def test_ci_and_blessed_alias_workflow_cover_every_target() -> None:
    ...
    for row in rows:
        ...
        assert row["triple"] in build, f"{ci['build_job']} no longer builds {row['triple']}"
        if ci["kind"] == "native":
            assert "_bootstrap-e2e.yml" in build, f"{ci['build_job']} lost native build/run coverage"
            assert ci["runner"] in build, f"{ci['build_job']} runner drifted"
            continue
        run = workflow_job(workflow, ci["run_job"])
        assert "_ci-cross-build-linux.yml" in build, f"{ci['build_job']} lost Linux cross-build coverage"
        assert "_ci-target-run.yml" in run, f"{ci['run_job']} lost native execution coverage"
        assert f"needs: {ci['build_job']}" in run, f"{ci['run_job']} is detached from its build job"
        assert row["triple"] in run and ci["runner"] in run, f"{ci['run_job']} target/runner drifted"
    ...
    assert matrix == expected, "blessed build alias matrix drifted from canonical-targets.json"
    assert '--target "${{ matrix.alias }}"' in blessed, "workflow no longer exercises aliases through soldr build"
...
def test_release_inclusions_and_exclusions_match_contract() -> None:
    ...
    assert archives == included, "release archive gate drifted from canonical-targets.json"
    for row in rows:
        release = row["release"]
        if release["status"] == "documented-exclusion":
            assert release["workflow_marker"] in workflow, f"release exclusion for {row['triple']} is undocumented"
...
def test_catalogue_mappings_cover_every_target() -> None:
    ...
    for row in rows:
        ...
        assert module.query_for_target(row["triple"]) == expected, f"cargo-nextest mapping drifted for {row['triple']}"
    ...
    for filename, constant in syslib_tables.items():
        ...
        for row in rows:
            assert mapping.get(row["triple"]) == row["catalogue"]["syslib_slug"], (

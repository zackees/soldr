...
SCRIPT = (
    Path(__file__).resolve().parents[1] / ".github" / "scripts" / "index_build_logs.py"
)
...
def test_indexes_logs_outside_a_logs_directory(indexer, tmp_path):
    ...
    (tmp_path / "logs" / "builds" / "build.xml").write_text(
        "<build/>", encoding="utf-8"
    )
    (tmp_path / "cache" / "zccache" / "history" / "17").mkdir(parents=True)
    (
        tmp_path / "cache" / "zccache" / "history" / "17" / "compile_journal.jsonl"
    ).write_text('{"unit":"a"}\n', encoding="utf-8")
    (tmp_path / "broker").mkdir()
    (tmp_path / "broker" / "broker-spawn.log").write_text("bound\n", encoding="utf-8")
    (tmp_path / "broker" / "broker-bringup.jsonl").write_text(
        '{"phase":"bind"}\n', encoding="utf-8"
    )
...
def test_build_state_directories_are_not_walked(indexer, tmp_path):
    ...
    (tmp_path / "toolchains" / "1.95.0" / "manifest.json").write_text(
        "{}", encoding="utf-8"
    )
    (tmp_path / "logs").mkdir()

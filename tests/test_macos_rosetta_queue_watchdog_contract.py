...
def test_x86_macos_replay_requires_rosetta_on_arm_runner() -> None:
    ...
    arm_replay = job_block("e2e-macos-arm64", "e2e-windows-x64-build")
    assert "runs_on: macos-15" in replay
    assert "target_execution: x86_64-rosetta" in replay
    assert "macos-15-intel" not in replay
    # soldr#2968: the translated x86 archive is the measured slow lane. Keep
    # its budget opt-in at the caller, rather than extending the reusable
    # target-run default or the native ARM replay sharing this runner image.
    assert "extended_replay: true" in replay
    assert "extended_replay:" not in arm_replay
    assert "default: false" in TARGET_RUN
    assert "inputs.extended_replay && 55 || 30" in TARGET_RUN
    assert "run_target_command.py --execution x86_64-rosetta --preflight" in TARGET_RUN
    assert (
        "run_target_command.py --execution '${{ inputs.target_execution }}'"
        in TARGET_RUN
    )

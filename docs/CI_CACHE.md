...
- **Native C/C++ compiler cache enabled by default.** Build-script work (bundled SQLite from `libsqlite3-sys`, `ring`, etc.) compiles through `cc-rs`, which `soldr cargo ...` now wraps with zccache by default so the same managed cache serves both rustc and native compilations. Cross-platform validation lives in the `native-sqlite` job in [`.github/workflows/cache-benchmark.yml`](../.github/workflows/cache-benchmark.yml); see [NATIVE_SQLITE_BENCHMARK.md](NATIVE_SQLITE_BENCHMARK.md) for matrix layout, stage breakdown, and how to read the per-target speedup tables. Native caching is on by default; the per-job opt-out lives in soldr's `SOLDR_NATIVE_CACHE` env var and is exposed by the corresponding setup-soldr input once it ships. The runtime kill-switch `soldr --no-cache cargo ...` still disables both layers at command time.

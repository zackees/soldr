...
- **Native C/C++ compiler cache enabled by default.** Build-script work (bundled SQLite from `libsqlite3-sys`, `ring`, etc.) compiles through `cc-rs`, which `soldr cargo ...` now wraps with zccache by default so the same managed cache serves both rustc and native compilations. Set `native-cache: false` on the setup step to write `SOLDR_NATIVE_CACHE=0` for the rest of the job and skip just the native wrapping; rustc-side caching stays on. The native-cache toggle does not affect the action-managed cache layers above; it changes how `soldr cargo ...` constructs the build subprocess environment. The runtime kill-switch `soldr --no-cache cargo ...` still disables both layers at command time.

...
| `SOLDR_DAEMON_RSS_CEILING_BYTES` (soldr#3038) | Optional diagnostic ceiling on `soldr-daemon`'s own resident set size, in bytes. Unset means no watchdog task is even spawned — zero behavior change for ordinary builds. When set to a positive integer, the daemon samples its own RSS every 2 s (`daemon::rss_ceiling::RSS_SAMPLE_INTERVAL`; deliberately not the 5-minute cache-maintenance cadence — see that module's docs) and records the running peak, whether the ceiling has been breached, and mimalloc's own exact `heap.committed`/`heap.detailed` counters alongside the OS RSS reading to `<SOLDR_CACHE_DIR>/cache/soldr-daemon/rss-ceiling-v1.json`. A breach is logged (`tracing::warn!`, event `daemon_rss_ceiling_breached`) and recorded, never enforced — the daemon keeps serving builds rather than exiting, because an exit here would fail the build in progress with a confusing "daemon unavailable" error instead of naming the real cause. Intended for CI/test use (see `daemon_rss_ceiling` in `crates/soldr-cli/tests/daemon/`), not for production ceilings. | unset (no ceiling, no watchdog) |

...
| `SOLDR_ZCCACHE_EMBEDDED` | Preview opt-in (issue #977 Phase 1 + 2). When set to the literal value `1`, the soldr-daemon process starts an in-process `zccache::embedded::ZccacheService` at boot instead of relying on the standalone managed `zccache` binary. The embedded service shares the daemon's tokio runtime so `console-subscriber` can span both products from a single attach. Strict parsing: any other value (`0`, `true`, `yes`, empty, unset) keeps today's wrapped-binary backend. Requires the `embedded` Cargo feature at compile time; without it the env var is read, a one-line warning is logged, and the backend remains `Wrapped`. Phase 1 + 2 wires the lifecycle only — wrapper invocations still talk to the managed binary. Phase 5 flips per-compile dispatch to the embedded service via a new IPC verb. | unset |

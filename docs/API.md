...
| `SOLDR_NESTED_CARGO` | Direct nested-Cargo guard permit (soldr#2924). The Cargo front door rejects build-like Cargo descendants that do not carry an explicit, provably distinct `--target-dir`, because Cargo's own `$CARGO` path bypasses Soldr's process-entry re-entrancy guard and can wait behind the outer Cargo's target lock. Set exactly `allow` on the **outer** invocation only when the nested orchestrator isolates its target through another mechanism (for example an env-only `CARGO_TARGET_DIR`) that process observation cannot verify. Other values do not disable the guard. | unset (guard enabled) |

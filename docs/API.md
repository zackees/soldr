...
| `SOLDR_NATIVE_GNU_LINK` | Whether a **host-native** `*-unknown-linux-gnu` build links through managed zig rather than the host's own glibc (soldr#2145). On by default so the x86_64 artifact gets the same 2.28 floor the cross-built aarch64 one already has, instead of inheriting whatever the build machine runs (2.39 on `ubuntu-24.04`). Set to `0`/`false`/`no`/`off` to restore host linking. Cross-compiles always use managed zig and ignore this. musl is unaffected — it is statically linked and has no glibc floor. | unset (enabled) |
